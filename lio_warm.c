...
op_status_t gen_warm_task(void *arg, int id)
{
    ...
    while ((gop = opque_waitany(q)) != NULL) {
        ...
        if (status.op_status == OP_STATE_SUCCESS) {
            ...
            nfailed++;
            wrid->bad++;
            warm_put_rid(db_rid, wrid->rid_key, w->inode, 1);
            j = gop_get_myid(gop);
...
//printf("argc=%d\n", argc);
    ...
    for (j=start_index; j<argc; j++) {
        ...
        it = lio_create_object_iter_alist(tuple.lc, tuple.creds, rp_single, ro_single, OS_OBJECT_FILE, recurse_depth, keys, (void **)vals, v_size, 3);
        if (it == NULL) {
