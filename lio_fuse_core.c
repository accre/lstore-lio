...
//  v_size = strlen(val);
    ...
    err = lio_set_attr(lfs->lc, lfs->lc->creds, (char *)fname, NULL, key, (void *)val, v_size);
    if (err != OP_STATE_SUCCESS) {
...
//  if ((lfs->enable_tape == 1) && ((ftype & OS_OBJECT_SYMLINK) == 0)) { //** Add the tape attribute
    ...
    log_printf(15, "bpos=%d size=" ST " buf=%s\n", bpos, size, buf);
...
#  endif
{
    ...
    v_size= size;
    log_printf(1, "fname=%s size=" ST " attr_name=%s\n", fname, size, name);
    flush_log();
