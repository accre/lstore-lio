...
op_generic_t *segjerase_write(segment_t *seg, data_attr_t *da, segment_rw_hints_t *rw_hints, int n_iov, ex_iovec_t *iov, tbuffer_t *buffer, ex_off_t boff, int timeout)
{
    ...
    for (i=0; i<n_iov; i++) {
        ...
        if ((rem_pos != 0) || (rem_len != 0)) {
            log_printf(1, "seg=" XIDT " offset/len not on stripe boundary!  data_size=%d off[%d]=" XOT " len[%d]=" XOT "\n",
...
int segjerase_deserialize_text(segment_t *seg, ex_id_t id, exnode_exchange_t *exp)
{
    ...
    if (slun->chunk_size != (s->chunk_size + JE_MAGIC_SIZE)) {
        log_printf(0, "Child chunk_size(" XOT ") != JE chunksize(%d) + JE_MAGIC_SIZE(%d)!\n", slun->chunk_size, s->chunk_size, JE_MAGIC_SIZE);
        inip_destroy(fd);
        return(-6);
    }
