        ...
        if (hints_list != NULL) {
            ...
            if (query_local != NULL) {
                ...
                if ((kvq_local.n_unique != 0) && (kvq_local.n_pickone != 0)) {
                    log_printf(0, "Unsupported use of pickone/unique in local RSQ hints_list[%d]\n", i);
            ...
            if (i<fixed_size) {  //** Use the fixed list for assignment
                ...
                if (rse == NULL) {
                    log_printf(0, "Missing element in hints list[%d]! Ignoring check.\n", i);
        ...
        for (j=0; j<rss->n_rids; j++) {
            ...
            if (op_state == NULL) {
                ...
                if (hints_list) {
                   ...
                   log_printf(1, "Match fail in fixed list and no hints are provided! i=%d\n", i);
                }
                status.op_status = OP_STATE_FAILURE;
